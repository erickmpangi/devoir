...
def write_summary_to_csv(total_file_sum, total_n_sum, total_execution_time_sum, start_time):
    # Écrire un résumé des totaux dans le fichier CSV à la fin
    with open('execution_times.csv', mode='a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([total_file_sum, total_n_sum, start_time.strftime("%Y-%m-%d %H:%M:%S"), total_execution_time_sum])
...
if __name__ == "__main__":
    ...
    # Créer un fichier CSV propre au début pour ne pas cumuler les anciens résultats
    ...
    for file_name in input_files:
        ...
        start_time_per_file = datetime.now()
        ...
        end_time = time.time()
        execution_time = end_time - start_time_per_file.timestamp()

        # Enregistrer le temps d'exécution, la date de début et le nombre de préférences dans le fichier CSV
        write_execution_time_to_csv(file_name.split('_')[1].split('.')[0], execution_time, start_time_per_file, num_preferences)

        # Mise à jour des totaux
        total_file_sum += int(file_name.split('_')[1].split('.')[0])
        total_n_sum += num_preferences
        total_execution_time_sum += execution_time

    # Écrire le résumé final avec les totaux dans le fichier CSV
    write_summary_to_csv(total_file_sum, total_n_sum, total_execution_time_sum, start_time)
